...
namespace SimpleLogger {
    static std::atomic_int next_thread_id{ 1 };
    static Level global_default_level = Level::Info;
    static thread_local std::string thread_name = "thread-" + std::to_string(next_thread_id.fetch_add(1, std::memory_order_relaxed));
    static Logger::Data data_root{""};
    ...
    namespace Impl {
            ...
            if (!name.empty()) {
                ...
                while(pos < name.size()) {
                    ...
                    if (it.second) {
                        ...
                        it.first->second->levels = data_root.levels;
                    }
        ...
        public:
            void add_emit_interface(std::unique_ptr<EmitInterface> ei, Configuration config) {
                ...
                std::lock_guard lock2(data_mutex);
                data_root.levels[index] = config.default_level;
                ...
                std::function<void(Logger::Data*)> update_level = [&](Logger::Data *data) {
                    for(auto i = 0u; i <= index; ++i) {
                        if (data->levels[i] == Level::None) {
                            data->levels[i] = data_root.levels[i];
                        }
                    }
